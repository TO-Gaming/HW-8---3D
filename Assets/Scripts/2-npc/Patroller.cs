...
public class Patroller: MonoBehaviour {
    ...
    [SerializeField] float braveradius = 5f;
    private void Start() {
    ...
    private void SelectNewTarget() {
        ...
        float[] newt = new float[allTargets.Length];
        for (int i=0; i<allTargets.Length; i++)
        {
            newt[i] = Vector3.Distance(allTargets[i].transform.position,playerPosition);
        }
        float max = newt.Max();
        int pmax = System.Array.IndexOf(newt, max);
        float min = newt.Min();
        int pmin = System.Array.IndexOf(newt, min);
        if (braveradius > distanceTofriend)
        {
            currentTarget = allTargets[pmin];
            Debug.Log("min");
        }
        else
        {
            currentTarget = allTargets[pmax];
            Debug.Log("max");
        }
